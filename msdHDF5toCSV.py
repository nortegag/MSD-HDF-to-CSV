...
Slightly modified to include further dimensions by Nicolas Ortega; all credit to Alexis Greenstreet.
...
def main():
    ...
    #the user, 
        ...
        ## add additional Rows to output csv:
        ## PICK HERE: use the corresponding getter function to get any desired variable (tough on some lists, long strings)
        csvRowString = ("SongID,AlbumID,AlbumName,ArtistID,ArtistLatitude,ArtistLocation,"+
            ...
            "Title,Year,"+
            "song_hotttnesss,artist_hotttnesss,artist_mbtags,artist_mbtags_count")
        #################################################
    ...
    for root, dirs, files in os.walk(basedir):        
        ...
        for f in files:
            ...
            ## additional dimensions: use the getter function to save the variable within the class
            song.song_hotness = str(hdf5_getters.get_song_hotttnesss(songH5File))
            song.artist_hotness = str(hdf5_getters.get_artist_hotttnesss(songH5File))
            song.artist_mbtags = str(hdf5_getters.get_artist_mbtags(songH5File))
            song.artist_mbtags_count = str(len(song.artist_mbtags))
            ...
            for attribute in csvAttributeList:
                ...
                ## adding attributes. Other ones may not be worth it, but possible to add
                elif attribute == 'song_hotttnesss':
                    song_hotness = song.song_hotness           
                    csvRowString += song_hotness
                elif attribute == 'artist_hotttnesss':
                    artist_hotness = song.artist_hotness  
                    csvRowString += artist_hotness
                elif attribute == 'artist_mbtags':
                    artist_mbtags = song.artist_mbtags
                    csvRowString += artist_mbtags
                elif attribute == 'artist_mbtags_count':
                    artist_mbtags_count = song.artist_mbtags_count
